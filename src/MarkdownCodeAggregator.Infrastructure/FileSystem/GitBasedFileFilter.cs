        ...
        : IFileFilter
{
    public async Task<IEnumerable<string>> GetTrackedFilesAsync(string directory)
    {
        try
        {
            ...
            if (exitCode == 0)
            {
                ...
                return files.Select(file => Path.Combine(directory,
                                                         file.Trim()));
            }

            logger.Warning("Git command failed. Falling back to directory scanning. Error: {Error}",
                            error);
        ...
        {
            logger.Error(ex,
                          "Error executing git ls-files command. Falling back to directory scanning.");
            return FallbackGetFiles(directory);
        }
    }
    ...
    public async Task<bool> ShouldIncludeFileAsync(string filePath, string baseDirectory)
    {
        try
        {
            ...
            logger.Debug("File {FilePath} is {IgnoreStatus} by Git",
                          filePath,
        ...
        {
            logger.Warning(ex,
                            "Error checking if file {FilePath} should be ignored. Including file by default.",
                            filePath);
    ...
    private bool IsDefaultIgnoredFile(string filePath)
    {
        ...
        return defaultIgnored.Any(ignored => filePath.Contains(ignored,
